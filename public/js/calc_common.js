...
function calc_01() {
	var before = $("calc_inp_01"), after = $("calc_inp_02");
...
function calc_02() {
	var basis = $("calc_inp_03"), basis_value = parseInt(basis.value.replaceAll(",", ""));
	if (basis.value == "") {
...
function calc_03() {
	var basis = $("calc_inp_04"), basis_value = parseInt(basis.value.replaceAll(",", ""));
	var month = $("calc_inp_05"), month_value = parseInt(month.value.replaceAll(",", ""));
	if (basis.value == "") {
	...
	var rate = new Array(), rate_corr = new Array(), rate_cost = new Array();
	for (var i=1; i<7; i++){
		rate[i] = 2.5*i;
		rate_cost[i] = conv_cost(basis_value, rate[i]);
...
function calc_04() {
	var basis = $("calc_inp_06"), basis_value = parseInt(basis.value.replaceAll(",", ""));
	var percent = $("calc_inp_07"), percent_value = parseFloat(percent.value.replaceAll(",", ""));
	if (basis.value == "") {
...
function calc_05() {
	var before = $("calc_inp_08"), before_value = parseInt(before.value.replaceAll(",", ""));
	var after = $("calc_inp_09"), after_value = parseInt(after.value.replaceAll(",", ""));
	var month = $("calc_inp_10"), month_value = parseInt(month.value.replaceAll(",", ""));
	if (before.value == "") {
		alert("기준 연봉 입력필드에 값을 입력해주세요.");
		before.focus();
		return false;
